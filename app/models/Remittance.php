...
class Remittance extends \Phalcon\Mvc\Model
{
    ...
    public function init($waybill_number, $amount, $company_regitration_number, $payer_id, $ref, $status){
        $this->setWaybillNumber(trim(strtoupper($waybill_number)));
    ...
    public static function fetchOne($id, $filter_by = 'id')
    {
        $obj = new Remittance();
        $builder = $obj->getModelsManager()->createBuilder()
            ...
            ->innerJoin('Parcel', 'Parcel.waybill_number = Remittance.waybill_number', 'Parcel')
            ->leftJoin('Admin', 'Admin.staff_id = Remittance.payer_id', 'Admin')
            ->where("Remittance.$filter_by = :id:");
    ...
    private static function filterConditions($filter_by)
    {
        ...
        if (isset($filter_by['ref'])) {
            $where[] = 'Remittance.ref = :ref:';
            $bind['ref'] = $filter_by['ref'];
        }
     ...
     * @return mixed
     */
    public static function fetchPaymentAdvice($offset, $count, $filter_by){
        $obj = new Remittance();
        $builder = $obj->getModelsManager()->createBuilder()
            ->from(array('c' => 'Company'));
        $builder->innerJoin('Remittance', 'r.company_registration_number = c.reg_no', 'r');
        ...
        if(isset($filter_by['company_registration_number'])){
            $where[] = 'r.company_registration_number = :company_registration_number:';
            $bind['company_registration_number'] = $filter_by['company_registration_number'];
        }
        if(isset($filter_by['ref'])){
            $where[] = 'r.ref = :ref:';
     ...
     * @return int|null
     */
    public static function countPaymentAdvice($filter_by){
        $obj = new Remittance();
        $builder = $obj->getModelsManager()->createBuilder()
            ->from(array('c' => 'Company'));
