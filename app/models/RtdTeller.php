...
/**
 * ReturnedShipmentTeller
 * 
 * @autogenerated by Phalcon Developer Tools
 * @date 2017-01-09, 16:14:09
 */
class RtdTeller extends \Phalcon\Mvc\Model
{
    ...
    {
        $this->hasMany('id', 'RtdTellerParcel', 'teller_id', array('alias' => 'RtdTellerParcel'));
    ...
    {
        return 'returned_shipment_teller';
    }
    ...
    public static function fetchAll($offset, $count, $filter_by, $fetch_with, $order_by_clause = null)
    {
        ...
        if ($order_by_clause != null) {
            ...
            $builder->orderBy('RtdTeller.modified_date');
        } else {
            $builder->orderBy('RtdTeller.id');
        }

        //model hydration
        if (isset($fetch_with['with_bank'])) {
            $columns[] = 'Bank.*';
            $builder->leftJoin('Bank', 'Bank.id = RtdTeller.bank_id');
        }
        if (isset($fetch_with['with_branch'])) {
            $columns[] = 'Branch.*';
            $builder->leftJoin('Branch', 'Branch.id = RtdTeller.branch_id');
        }
        if (isset($fetch_with['with_payer'])) {
            $columns[] = 'Admin.*';
            $builder->leftJoin('Admin', 'Admin.id = RtdTeller.paid_by');
        }
        ...
        if (isset($filter_by['teller_no'])) {
            ...
            $builder->inWhere('RtdTeller.teller_no', $teller_no);
        }
    ...
    public static function fetchOne($id, $in_recursion = false)
    {
        $obj = new RtdTeller();
        $builder = $obj->getModelsManager()->createBuilder()
            ->columns(['RtdTeller.*', 'Parcel.*'])
            ->from('RtdTeller')
            ->leftJoin('RtdTellerParcel', 'RtdTellerParcel.teller_id = RtdTeller.id', 'RtdTellerParcel')
            ->leftJoin('Parcel', 'Parcel.id = RtdTellerParcel.parcel_id', 'Parcel')
            ->where('RtdTeller.id = :id:');
        ...
        try {
            ...
            if ($check) {
                foreach ($parcel_id_array as $parcel_id) {
                    $teller_parcel = new RtdTellerParcel();
            ...
            Util::slackDebug('Rtd Teller no saved', $e->getPrevious()?$e->getPrevious()->getMessage():$e->getMessage());
        }
