...
class UpdateBulkParcelCreationJobStatus extends AbstractMigration
{
    ...
    {
       $jobIds =  $this->fetchAll("SELECT DISTINCT j.id FROM jobs j LEFT JOIN bulk_shipment_job_details bsjd ON (j.id = bsjd.job_id AND bsjd.status = 'success') WHERE bsjd.id IS NULL AND j.queue = 'bulk_shipment_creation'");
